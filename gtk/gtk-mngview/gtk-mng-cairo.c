		    ...
		    GdkRectangle * area)
{
  ...
  printf("ROWSTRIDE %d area width %d mngview width %d area height %d mngview height %d\n", rowstride, area->width, mng_view->width, area->height, mng_view->height);
  ...
  while (bptr < buffer + rowstride * area->height)
    {
        for (i = 0, j=0 ; (i + j)< rowstride; i+=4, j++) {
            bptr[i] = aptr[j];
            bptr[i + 1] = ptr[i];
            bptr[i + 2] = ptr[i+1];
            bptr[i + 3] = ptr[i+2];
...
#if 1
  cairo_surface_t *surface = cairo_image_surface_create_for_data(buffer, CAIRO_FORMAT_ARGB32, area->width, area->height, rowstride);

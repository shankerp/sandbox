                                                  ...
                                                  mng_uint32  iItermax)
{
 ...
 mng_bool   mng_process_arow_callback   (mng_handle  hHandle,
                                                  mng_uint32  iRowsamples,
                                                  mng_bool    bIsRGBA16,
                                                  mng_ptr     pRow)
{
    printf("PROCESSED A ROW\n");
    return MNG_TRUE;
}
...
gtk_mng_view_animator (GtkMngView * mng_view)
{
  ...
  if (retcode == MNG_NOERROR)
    ...
    g_warning ("mng_display_resume() return not good value %d", retcode);
			...
			mng_uint32 delay)
{
  ...
  int i = 0, j = 0;
  mng_view = GTK_MNG_VIEW (mng_get_userdata (mng_h));
  printf("Delay %d width %d height %d \n", delay, mng_view->width, mng_view->height);
  //for (i = 0; i < mng_view->height; i++) {
  //    for(j = 0; j < 3 * mng_view->width; j++) 
  //      printf("%x ", (mng_view->MNG_drawing_buffer + i * 3 * mng_view->width)[j]);
  //    printf("\n");
  //}
			      ...
			      mng_uint32 line)
{
  ...
  //printf("Canvas Line %d height %d width %d\n", line, mng_view->height, mng_view->width);
  FUNCTION_EXIT();
...
mng_bool mng_processarow_callback   (mng_handle  hHandle,
                                                  mng_uint32  iRowsamples,
                                                  mng_bool    bIsRGBA16,
                                                  mng_ptr     pRow)
{
    printf("Row Samples %d\n", iRowsamples);
    printf("Is RGBA16 %d\n", bIsRGBA16);
    sleep(10);
    return MNG_TRUE;
}

mng_bool mng_processgamma_callback  (mng_handle  hHandle,
                                                  mng_uint32  iGamma)
{
    printf("Gamma is %d\n", iGamma);
    return MNG_TRUE;
...
mng_bool mng_processchroma_callback  (mng_handle  hHandle,
                                                  ...
                                                  mng_uint32  iBluey)
{
    printf(" iWhitepointx,is %d\n", iWhitepointx);
    printf(" iWhitepointy,is %d\n", iWhitepointy);
    printf(" iRedx,       is %d\n", iRedx);
    printf(" iRedy,       is %d\n", iRedy);
    printf(" iGreenx,     is %d\n", iGreenx);
    printf(" iGreeny,     is %d\n", iGreeny);
...
mng_bool mng_processrgb_callback  (mng_handle  hHandle,
                                                  mng_uint32  iGamma)
{
    printf("rgb is %d\n", iGamma);
    return MNG_TRUE;
...
mng_bool mng_processiccp_callback  (mng_handle  hHandle,
...
void
display_mng (GtkMngView * mng_view)
{
